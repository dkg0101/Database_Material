create database ineuron_fsds;
show databases;
create database if not exists ineuron_fsds # to check if tabale ,databases already exists and avoid error use 'if not exists'
use ineuron_fsds;
...
poutcome varchar(30) , 
y varchar(30)
) # we can use default mysql reserved keywords as column name by using `column name` symbol
select * from bank_details
insert into bank_details values(33,"services","married","secondary","no",
4789,"yes","yes","cellular",11,"may",220,1,339,4,"failure","no"	)	;
...
select age,job,loan,`day`,`month`from bank_details
select Age,Job from bank_details #Not case sensitive

#To select first n records, n=10
select * from bank_details limit 10

#Conditional fetching
select * from bank_details where age>=35
select * from bank_details where age=40
select * from bank_details where age=60 and job = 'retired'
select * from bank_details where age=60 and marital = 'single'
...
#Arrange based on order of column
...
#having clause
select * from bank_details having min(balance)
...
#person with maximum balance
select * from bank_details where balance = (select max(balance) from bank_details)
call max_bal() 

#Persoon having loan
select * from bank_details where loan = 'yes'
select * from bank_details having loan='yes'

#avg balance of all people whoese job roll is admin
...
#find bank record having negative balance
select * from bank_details where balance<0

#record not having house along with balance
